...
/** The complete, reusable device-connection experience for a subscription. */
export function ConnectionExperience({ subscriptionId, onGoBack }: ConnectionExperienceProps) {
  ...
  const openDeepLink = useCallback(
    (deepLink: string) => {
      let resolved = hasTemplates(deepLink) ? resolveUrl(deepLink) : deepLink;
      if (
      ...
      const finalUrlForTelegram = /^https?:\/\//i.test(resolved)
        ? resolved
      ...
      if (isTelegramWebApp) {
        try {
          ...
          /* SDK unavailable; use browser fallback. */
        }
      }
  ...
  const hasApps = useMemo(
    () =>
      Object.values(appConfig?.platforms ?? {}).some(
        (platform: RemnawavePlatformData) => platform.apps?.length > 0,
      ),
    [appConfig?.platforms],
  );
