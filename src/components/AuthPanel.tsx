...
import { getAndClearReturnUrl, isValidRedirectUrl, tokenStorage } from '../utils/token';
...
export default function AuthPanel({ returnTo, initialMode, embedded = false }: AuthPanelProps) {
  ...
  const safeReturnTo = returnTo && isValidRedirectUrl(returnTo) ? returnTo : undefined;
  ...
  const getReturnUrl = useCallback(() => {
    ...
    if (stateFrom && stateFrom !== '/login' && isValidRedirectUrl(stateFrom)) return stateFrom;
    const savedUrl = getAndClearReturnUrl();
    if (savedUrl && savedUrl !== '/login' && isValidRedirectUrl(savedUrl)) return savedUrl;
    return '/';
  }, [location.state, safeReturnTo]);
  ...
  return (
    <div
      className={
        embedded ? 'w-full' : 'flex min-h-[100dvh] items-center justify-center px-4 sm:px-6 lg:px-8'
      }
    >
