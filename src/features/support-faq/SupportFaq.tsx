...
export function SupportFaq() {
  ...
  const contentId = 'support-faq-content';
  const toggleId = 'support-faq-toggle';
  ...
  return (
    <Card>
      <button
        ...
        className="flex min-h-[52px] w-full items-center justify-between gap-3 text-left focus:outline-none focus:ring-2 focus:ring-accent-500/60"
        aria-expanded={isExpanded}
        aria-controls={contentId}
      >
        <span>
          <span className="block text-lg font-semibold text-dark-100">{t('support.faqTitle')}</span>
          <span className="mt-1 block text-sm text-dark-400">{t('support.faqDescription')}</span>
        </span>
        <PiCaretDown
          className={`h-5 w-5 shrink-0 text-dark-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
          aria-hidden="true"
        />
      </button>
      {isExpanded && (
        <div id={contentId} role="region" aria-labelledby={toggleId} className="mt-4 space-y-2">
          {items.map((item) => {
