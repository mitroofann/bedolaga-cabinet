...
Пока лендинг-воронки нет, **все** кнопки (покупка, «Попробовать», вход) ведут на
`https://cabinet.bulkavpn.net/login`. Когда появится воронка, кнопки покупки можно будет
перенаправить на `/buy/<slug>` (см. историю — раньше вели на `/buy/now?intent=trial`).
