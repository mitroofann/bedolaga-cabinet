...
Кнопки ведут в кабинет `https://cabinet.bulkavpn.net` по назначению:

- «Попробовать за 10 ₽» → `/buy/now?intent=trial` (воронка Bulka, вкладка пробного периода).
- «Купить VPN» / «Выбрать тариф» / «Выбрать» / «Оформить подписку» → `/buy/now` (воронка Bulka, покупка).
- «Войти в ЛК» / «Войти в аккаунт» → `/login`.

`now` — slug лендинг-воронки Bulka в кабинете (`/buy/:slug`).

> **Проверь домен:** в кнопках зашит `cabinet.bulkavpn.net`, а кабинет у тебя на
