...
- **Кнопки покупки** («Купить VPN», «Выбрать тариф», «Выбрать», «Оформить подписку») →
  `/buy/now` — публичная воронка покупки (регистрация заранее не нужна, аккаунт
  создаётся после оплаты).
- **Кнопки «Попробовать за 10 ₽»** → `/buy/now?intent=trial` — та же воронка, но блок
  триала подсвечивается и оказывается в фокусе (если триал включён на лендинге в админке).
